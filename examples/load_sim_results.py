...
@ray.remote
def get_sim_results_ray(csv_path,file_name):
    print('Loading: '+file_name)
    flow_vehicle_data = get_vehicle_data(csv_path)
    ...
    p = get_sim_params(file_name)
    
    x = [p[0],p[1],p[2],p[3],total_fuel_consumption,total_distance_traveled,total_travel_time,total_speed_var,number_veh,number_ACCs]
    
    return x

def get_sim_results(csv_path,file_name):
    flow_vehicle_data = get_vehicle_data(csv_path)
...
def get_all_sim_results(sims_pathm,want_parallel=True):
...
if __name__ == "__main__":
    ...
    sim_results = get_all_sim_results(sims_path,want_parallel=False)
    
    np.savetxt('sim_results_ver_1.csv', sim_results)
